...
本目录保存前端冻结实施方案、前端工程规范和各阶段实际工作记录。前端源代码位于仓库根目录 `support-agent-web/`，当前已完成阶段 F1～F2；判断实际能力时必须结合最近阶段记录，不得把后续阶段计划写成已完成功能。
...
| 阶段 F2 | 已完成 | POST SSE 聊天、会话生命周期、引用、工单建议和长期记忆治理已通过门禁 |
| 阶段 F3～F5 | 待执行 | 工单、管理员治理和最终 E2E 尚未实现 |
| 本地联调 | F1～F2 已完成 | 认证、真实 DashScope 对话、会话和长期记忆已连接本地后端验证 |
