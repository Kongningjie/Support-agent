...
> 基线日期：2026-09-21；对应代码：阶段 16 完成后；用途：后续规划、开发、Review 和 AI Coding 的现状入口。

## 1. 完成状态

- 一期阶段 0～5、二期阶段 6～9、三期阶段 10～14 及阶段 13 补强批次均已完成规定离线与集成门禁。
- 最近门禁结果：`mvn test` 通过 235 个测试；阶段 16 的 `mvn verify -Pintegration` 通过 47 个 MySQL、Redis、Elasticsearch 和应用启动集成测试。
- 阶段 10 的真实在线摘要质量评测按用户决定暂缓；不得据此宣称在线摘要质量已经验收。
- 四期 LLM 安全优化方案已经冻结，阶段 15～16 已完成，阶段 17 待执行；未收到明确阶段执行指令前不得开始下一阶段。

## 2. 当前能力

系统已经实现托管知识、混合检索、Rerank、带引用 SSE 回答、工单和案例闭环、离线评测、滚动摘要、本地用户认证、资源归属、会话列表/详情/重置/删除、用户可控长期记忆、候选清理与并发治理、账号锁定、改密、管理员用户治理和安全事件审计。

当前 LLM 安全能力仍属于分阶段覆盖：阶段 15 已为当前消息、历史、摘要、长期记忆、检索证据和工单字段建立确定性 `ALLOW/GUARD/BLOCK` 判定与转义数据区；高置信度直接注入在会话创建前以 422 拒绝，高风险上下文只从本次调用排除。阶段 16 已让问候、RAG、工单回答、工单草稿和案例草稿统一经过 `PASS/REGENERATE/REJECT` 输出网关；每次受保护生成调用使用独立随机泄漏标记，并在外发或持久化前检查 Prompt 片段、角色标记、凭据、不安全协议、身份证、手机号和受限邮箱。可修复失败最多完整重生成一次，失败正文不会形成 `answer.delta`、半轮会话或业务草稿。固定中文对抗评测、低基数安全指标和真实模型对抗验证仍待阶段 17 实现。
...
- 四期必须严格按阶段 15、16、17 顺序执行；方案中的安全能力在对应阶段通过测试和 Review 前仍视为未实现。
