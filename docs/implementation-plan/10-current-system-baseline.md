...
> 基线日期：2026-09-26；对应代码：后端阶段 17、前端阶段 F2 完成后；用途：后续规划、开发、Review 和 AI Coding 的现状入口。
...
- 前端阶段 F1～F2 已完成：除认证、账号安全和应用外壳外，已提供真实 POST SSE 聊天、引用、会话生命周期和用户可控长期记忆；F3～F5 尚未执行。

## 2. 当前能力

系统已经实现托管知识、混合检索、Rerank、带引用 SSE 回答、工单和案例闭环、离线评测、滚动摘要、本地用户认证、资源归属、会话列表/详情/重置/删除、用户可控长期记忆、候选清理与并发治理、账号锁定、改密、管理员用户治理和安全事件审计。独立前端当前能够完成登录、认证恢复、强制改密、本人账号安全、两级角色页面边界、聊天与引用、会话生命周期及长期记忆治理；工单和管理员业务页面尚未实现。
