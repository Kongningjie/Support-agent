...
| 2 | 工单、幂等与异步任务核心 | 已完成 | 无 AI 依赖的工单主流程和持久化任务机制可用 |
| 3 | 托管知识与 Elasticsearch 索引 | 已完成 | 文档可安全导入、异步发布、归档并进入版本化索引 |
| 4 | 混合检索、Agent 与 SSE 对话 | 已完成 | BM25、向量、RRF、Rerank、受控 Agent 与安全 SSE 主链路已通过门禁 |
| 5 | 案例闭环、评估与一期验收 | 已完成 | 工单解决、案例审核发布、固定检索评测和一期质量门禁已完成 |

后续进度：二期阶段 6～9 全部完成；三期阶段 10～14 及阶段 13 补强批次全部完成；四期阶段 15～17 全部完成规定离线与集成门禁。第五期轻量级学习部署阶段 18 已冻结、尚未执行。阶段 10 的真实在线摘要质量评测由用户决定暂缓，阶段 17 的真实模型对抗验证因本阶段未获得单独在线授权而保留为未验证项。
...
一期阶段 0～5 已全部完成。后续质量、性能和稳定性优化统一依据 [Support Agent 二期优化实施计划](08-phase-2-optimization-plan.md) 执行，其中阶段 6～9、D1～D14、指标口径和验收门禁已经冻结。

执行阶段 6～9 时，一期冻结架构、业务事实、安全边界和接口契约继续有效；二期文档仅在明确写出的优化范围内补充一期方案。发生冲突时必须暂停并请求确认，不得自行扩大技术栈或业务范围。

```text
请执行 docs/implementation-plan/08-phase-2-optimization-plan.md 的第 N 阶段。
严格遵守 docs/standards/ 下的规范，只实现本阶段内容。
完成后执行规定测试和完整 Review，更新阶段工作记录；不要提前执行下一阶段，也不要自动提交或推送。
```
...
二期阶段 6～9 已全部完成。三期阶段 10～14 统一依据 [Support Agent 三期用户与记忆治理实施计划](09-phase-3-conversation-memory-plan.md) 逐阶段执行，依次交付单会话上下文治理、用户认证与资源归属、会话生命周期、用户可控长期记忆以及账号安全与认证演进边界。

阶段 10～14 及阶段 13 补强批次均已完成规定离线与集成门禁。阶段 10 的真实在线摘要质量评测按用户决定暂缓并保留为工作记录中的未验证项。后续新增阶段必须先形成冻结方案，不得直接从历史计划推断并扩展业务范围。

```text
请执行 docs/implementation-plan/09-phase-3-conversation-memory-plan.md 的第 N 阶段。
严格遵守 docs/standards/ 下的规范，只实现本阶段内容。
完成后执行规定测试和完整 Review，更新阶段工作记录；不要提前执行下一阶段，也不要自动提交或推送。
```
...
第五期只用于获得第一次完整云服务器部署经验，统一依据 [Support Agent 五期轻量级学习部署实施计划](12-phase-5-lightweight-deployment-plan.md) 执行阶段 18。其冻结目标为单台云服务器、Docker Compose 运行基础设施、系统服务运行 Spring Boot JAR，并通过 SSH 隧道使用 Apifox、Postman 或 IDEA HTTP Client 验证后端。

阶段 18 不以前端为前置条件，也不实现公网业务访问、HTTPS、高可用、CI/CD、集中监控或生产 SLA。历史生产准备建议不自动进入本阶段范围。
