...
public class RedisConversationStoreAdapter implements ConversationStorePort {
    ...
    public BeginResult begin(UUID conversationId, UUID clientMessageId, String message,
        ...
        String script = """
                ...
                if oldHash and oldHash~=ARGV[2] then return {'MESSAGE_REUSED'} end
                ...
                if exists==0 then
                  if ARGV[3]~='-' then return {'EXPIRED'} end
                  redis.call('HSET',KEYS[1],'version','0','lastSequence','0')
                else
                  local version=redis.call('HGET',KEYS[1],'version') or '0'
                  if ARGV[3]=='-' or version~=ARGV[3] then return {'VERSION',version} end
                end
                local owner=redis.call('HGET',KEYS[1],'activeRunId')
                local lease=tonumber(redis.call('HGET',KEYS[1],'runLeaseUntil') or '0')
                if owner and lease>tonumber(ARGV[4]) then return {'BUSY'} end
                local interrupted=''
                if owner then interrupted=owner end
                redis.call('HSET',KEYS[1],'status','RUNNING','activeRunId',ARGV[1],
                  'runLeaseUntil',ARGV[5],'lastAccessAt',ARGV[4],'activeMessageKey',KEYS[2])
                redis.call('HSET',KEYS[2],'requestHash',ARGV[2],'status','PROCESSING','runId',ARGV[1])
    ...
    /** {@inheritDoc} */
    @Override
    public boolean renew(UUID conversationId, UUID runId, Instant now) {
        String script = "if redis.call('HGET',KEYS[1],'activeRunId')~=ARGV[1] then return 0 end "
    ...
    /** {@inheritDoc} */
    @Override
    public long nextSequence(UUID conversationId, UUID runId) {
        String script = "if redis.call('HGET',KEYS[1],'activeRunId')~=ARGV[1] then return -1 end "
    ...
    /** {@inheritDoc} */
    @Override
    public long nextReplaySequence(UUID conversationId) {
        Long result = redis.opsForHash().increment(conversationKey(conversationId), "lastSequence", 1);
    ...
    /** {@inheritDoc} */
    @Override
    public CompletedTurn complete(UUID conversationId, UUID runId, CompletedTurn turn,
        ...
        String script = """
                ...
                redis.call('HSET',KEYS[2],'status','COMPLETED','result',ARGV[2])
                if ARGV[6]=='1' then
                  redis.call('HSET',KEYS[4],'status','AVAILABLE','context',ARGV[7],
                    'sourceTurnId',ARGV[8]); redis.call('EXPIRE',KEYS[4],ARGV[10])
                end
                redis.call('EXPIRE',KEYS[1],ARGV[9]); redis.call('EXPIRE',KEYS[2],ARGV[9]);
                redis.call('EXPIRE',KEYS[3],ARGV[9]); return 1
                """;
        Long result = redis.execute(new DefaultRedisScript<>(script, Long.class),
                List.of(conversationKey(conversationId), messageKey(conversationId, turn.clientMessageId()),
                        turnsKey(conversationId), suggestionKey), runId.toString(), write(turn),
    ...
    /** {@inheritDoc} */
    @Override
    public void fail(UUID conversationId, UUID runId, Instant now) {
        String script = "if redis.call('HGET',KEYS[1],'activeRunId')==ARGV[1] then "
                + "local messageKey=redis.call('HGET',KEYS[1],'activeMessageKey'); "
                + "redis.call('HSET',KEYS[1],'status','IDLE','lastAccessAt',ARGV[2]); "
                + "redis.call('HDEL',KEYS[1],'activeRunId','runLeaseUntil','activeMessageKey'); "
        ...
        redis.execute(new DefaultRedisScript<>(script, Long.class),
                ...
                runId.toString(), Long.toString(now.toEpochMilli()));
    }

    /** {@inheritDoc} */
    @Override
    public SuggestionClaim claimSuggestion(UUID conversationId, UUID suggestionId, Instant now) {
        ...
        String script = """
                ...
                if status=='PROCESSING' and lease>tonumber(ARGV[1]) then return {'PROCESSING'} end
                redis.call('HSET',KEYS[1],'status','PROCESSING','claimId',ARGV[2],'leaseUntil',ARGV[3])
                return {'CLAIMED',ARGV[2],redis.call('HGET',KEYS[1],'context'),redis.call('HGET',KEYS[1],'sourceTurnId'),''}
                """;
    ...
    /** {@inheritDoc} */
    @Override
    public void consumeSuggestion(UUID conversationId, UUID suggestionId, UUID claimId,
                                  String ticketNo, Instant now) {
        String script = "if redis.call('HGET',KEYS[1],'claimId')~=ARGV[1] then return 0 end "
                + "redis.call('HSET',KEYS[1],'status','CONSUMED','ticketNo',ARGV[2],'consumedAt',ARGV[3]); "
                + "redis.call('HDEL',KEYS[1],'claimId','leaseUntil','context'); return 1";
        Long result = redis.execute(new DefaultRedisScript<>(script, Long.class),
                List.of(suggestionKey(conversationId, suggestionId)), claimId.toString(), ticketNo,
    ...
    /** {@inheritDoc} */
    @Override
    public List<String> recentContext(UUID conversationId, int maximumTurns, int maximumCharacters) {
        List<String> json = redis.opsForList().range(turnsKey(conversationId), -maximumTurns, -1);
